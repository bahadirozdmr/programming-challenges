...
The first part of the array will be a list of vertices in a graph in the form (A,B,C,...),
 the second part of the array will be the edges connecting the vertices in the form (A-B,C-D,...)
  ...
  and your program will have to determine whether or not the set of vertices given covers every edge in the graph such that every edge is incident to at least one vertex in the set.
...
public class VertexConvering {

    public String covering(String[] strArrays) {

        String result="";
        var connections=getConnections(strArrays[1]) ;
        var searchNodes=getSearchNodes(strArrays[2]);
        for(String connection:connections) {
            if(!isNodeExists(connection,searchNodes)) {
    ...
    boolean isNodeExists(String connection,String[] nodes) {
    ...
    public String[] getConnections(String connectionStr){
        connectionStr=connectionStr.replace("(","").replace(")","");
        return connectionStr.split(",");
    }
    public String[] getSearchNodes(String searchNodes) {
        searchNodes=searchNodes.replace("(","").replace(")","");
        return searchNodes.split(",");
    }
